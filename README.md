...
## 开发日志 (Changelog)
### [v1.4.0] - 2026-03 - 全面云端化与体验重构 (Cloud Integration & UX Overhaul)

这是本项目迄今为止最核心的一次架构升级。系统已彻底告别本地存储，全面接入云端数据库，并对底层编辑器与交互细节进行了深度重构。

#### ✨ 核心特性 (Features)
* **全面云端同步**：完整接入 Supabase 后端服务。月历、项目、博客、白板四大模块的数据现已实现云端持久化存储与多端实时同步。
* **全新账号体系**：引入了定制化的身份鉴权系统。支持“专属账号名+邮箱”注册，并支持账号名或邮箱双路极简登录。
* **全新 Markdown 引擎**：博客模块彻底弃用富文本 HTML 标签，重构为纯正的 Markdown 编辑器（基于 Vditor），支持代码高亮、原生 MD 语法及沉浸式全屏输入。
* **新用户冷启动引导**：为首次注册的用户自动注入专属示例数据（包含预置项目、日志、博客说明及白板便签），配合全新的商业级欢迎弹窗，大幅降低上手门槛。

#### ⚡ 性能与底层优化 (Performance & Architecture)
* **乐观更新 (Optimistic UI)**：全面重写 Pinia 状态管理逻辑，增删改查操作在前端瞬间响应，后台静默落库，彻底消除网络延迟带来的卡顿感。
* **高频操作防抖保护**：为日志输入（500ms）和白板便签拖拽（300ms）引入坐标与输入防抖机制，极大节省了云端数据库的 API 请求开销。
* **纯净数据导出**：废弃繁琐的本地 JSON 备份，设置页新增“原生 Markdown 导出”功能，一键将四大版面的核心数据排版为极其干净的 `.md` 文档。

#### 🎨 界面与体验打磨 (UI/UX Improvements)
* **视觉一致性**：统一四大版面的头部排版与边距规范，彻底消除页面切换时的视觉跳动。
* **全局图标重绘**：侧边栏导航图标由“线框风”全面升级为“实心扁平风 (Solid)”，更具现代 App 质感。
* **沉浸式交互弹窗**：废除浏览器原生的 `confirm` 告警，为“删除项目”、“删除博客”、“退出登录”等破坏性操作定制了统一风格的内置二次确认弹窗。

---

### [v1.3.0] - 2026-03 - 移动端深度适配 (Mobile Responsive Update)

本次更新聚焦于跨设备体验，让系统在手机等小屏幕设备上依然保持优雅的排版与完整的核心功能。

#### 📱 移动端适配亮点
* **底部导航重构**：在屏幕宽度小于 768px 时，左侧边栏将自动无缝转换为固定在底部的“App 级 Tab 导航栏”，释放更多垂直阅读空间。
* **弹性视口保护**：通过全局动态高度计算与底层 CSS 拦截，彻底修复了移动端浏览器因工具栏收放导致的页面抖动与溢出问题。
* **卡片响应式布局**：优化了“项目看板”与“设置中心”的网格布局，在小屏下自动切换为大卡片单列流式布局，提升触控点击体验。
* **防误触优化**：禁用了移动端导航栏的悬停状态（Hover）残留，优化了按钮的触控反馈区域。
